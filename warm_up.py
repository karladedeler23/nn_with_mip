...
for class_label in range(n): # Iterate through the dataset to select one data point per class
    index = np.where(y_train == (class_label%10))[0][class_label]#np.random.randint(100)]  # Get the index of one of the occurrences of the class
...
### Train with typical SGD (keras model)
...
# Example data (first 5 samples from the test set)
x_samples = X_train_sample
y_samples = y_train_one_hot

# Forward pass to calculate the output
hidden_layer_output = np.maximum(0, np.dot(x_samples, sgd_w[0]) + sgd_b[0])  # ReLU activation
...
#print(f"Keras predictions loss: {model_sgd.predict(x_samples)}")

# Calculate hinge loss manually
hinge_loss_per_class = np.maximum(1.0 - (2*y_samples-1) * output_layer_output, 0.0)
hinge_loss = np.sum(hinge_loss_per_class)
print(f"Manual hinge loss: {hinge_loss}")

# Compare with Keras evaluation
loss, accuracy = model_sgd.evaluate(x_samples, y_samples, verbose=0)
...
for j in range(hidden_layers[0]):
    biases[0][j].start = sgd_b[0][j]
    #biases[0][j].lb = biases[0][j].ub = sgd_b[0][j]
    for k in range(input_dim):
        weights[0][j, k].start = sgd_w[0][k,j]
        #weights[0][j, k].lb = weights[0][j, k].ub = sgd_w[0][k,j]
for l in range(1, len(hidden_layers)):
     for j in range(hidden_layers[l]):
        biases[l][j].start = sgd_b[l][j]
        #biases[l][j].lb = biases[l][j].ub = sgd_b[l][j]
        for k in range(hidden_layers[l-1]):
            weights[l][j, k].start = sgd_w[l][k,j]
            #weights[l][j, k].lb = weights[l][j, k].ub = sgd_w[l][k,j]
for j in range(output_dim):
    biases[-1][j].start = sgd_b[-1][j]
    #biases[-1][j].lb = biases[-1][j].ub = sgd_b[-1][j]
    for k in range(hidden_layers[-1]):
        weights[-1][j, k].start = sgd_w[-1][k,j]
        #weights[-1][j, k].lb = weights[-1][j, k].ub = sgd_w[-1][k,j]
...
# If the model is infeasible, run the IIS finder
if model.status == GRB.INFEASIBLE:
    print("Model is infeasible, finding IIS...")
    model.computeIIS()
    model.write("infeasible_debugging.ilp")
